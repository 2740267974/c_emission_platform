...
import 'package:CEP/beans/system_option_new_i_bean.dart';
...
//默认值
SystemOptionBean10? systemOptionBean10;
...
class SysOptionSettingPage10 extends StatelessWidget {

  //设置该页面路由名称字符串常量
  //路由是指路由器从一个接口上收到数据包，根据数据包的目的地址进行定向并转发到另一个接口的过程
  //定义一个静态常量sysOptionSettingPageRouterName，其值为字符串"/SysOptionSettingPage"
  static const sysOptionSettingPageRouterName = "/SysOptionSettingPage10";
  ...
  const SysOptionSettingPage10({super.key});
...
class _MySysOptionSettingState extends State<MySysOptionSettingPage> {
  final List<String> options= fuelMill;
  @override
  ...
  void initState() {
    ...
    _controlers = List.generate(4, (_) => TextEditingController());
    systemOptionBean10  =
        DataProvider.getInstance()?.systemOptionBean10;
    if(systemOptionBean10 != null){
      _controlers[0].text =
      systemOptionBean10 ?.meimo_kw.toString() as String;
      _controlers[1].text =
      systemOptionBean10  ?.meimo_product.toString() as String;
      _controlers[2].text =
      systemOptionBean10 ?.xuanfenji_kw.toString() as String;
      _controlers[3].text =
      systemOptionBean10 ?.xuanfenji_product.toString() as String;
    }
  }
...
void save() {
  ...
  try {
    SystemOptionBean10 systemOptionBean10 = SystemOptionBean10(
    ...
    DataProvider?.setSystemOptionBean10(systemOptionBean10);
  } catch (e) {
    // 处理异常的代码
    print('Error: $e');
  }
...
void getValue() {
  String message = (DataProvider?.getSystemOptionBean10() as SystemOptionBean10).toString();
