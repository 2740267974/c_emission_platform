...
import 'package:CEP/beans/system_option_new_h_bean.dart';
...
//默认值
SystemOptionBean8? systemOptionBean8;
SystemOptionBean9? systemOptionBean9;
...
class SysOptionSettingPage9 extends StatelessWidget {

  //设置该页面路由名称字符串常量
  //路由是指路由器从一个接口上收到数据包，根据数据包的目的地址进行定向并转发到另一个接口的过程
  //定义一个静态常量sysOptionSettingPageRouterName，其值为字符串"/SysOptionSettingPage"
  static const sysOptionSettingPageRouterName = "/SysOptionSettingPage9";
  ...
  const SysOptionSettingPage9({super.key});
...
class _MySysOptionSettingState extends State<MySysOptionSettingPage> {
  final List<String> options= fuelClass;
  final List<String> options2= substituteFuel;
  @override
...
void save() {
  ...
  try {
    //这部分代码用于创建一个OptionBean2对象，其中syTypes.indexOf(selectedOption!)表示在下拉菜单中选择的值的索引
    //double.parse(_controlers[0].text)和double.parse(_controlers[1].text)表示将输入框中的文本转换为double类型
    SystemOptionBean8 systemOptionBean8=SystemOptionBean8(fuelClass.indexOf(selectedOption1!));
    DataProvider?.setSystemOptionBean8(systemOptionBean8);
    SystemOptionBean9 systemOptionBean9=SystemOptionBean9(substituteFuel.indexOf(selectedOption2!));
    DataProvider?.setSystemOptionBean9(systemOptionBean9);
  } catch (e) {
    // 处理异常的代码
    print('Error: $e');
  }
