...
class SysOptionSettingPage3 extends StatelessWidget {
  ...
  const SysOptionSettingPage3({super.key});
...
class _MySysOptionSettingState extends State<MySysOptionSettingPage> {
  ...
  void initState() {
    ...
    _controlers = List.generate(3, (_) => TextEditingController());
    systemOptionBean2  =
        DataProvider.getInstance()?.systemOptionBean2;
    if(systemOptionBean2 != null){
      _controlers[0].text =
      systemOptionBean2 ?.clinker.toString() as String;
      _controlers[1].text =
      systemOptionBean2  ?.plaster.toString() as String;
      _controlers[2].text =
      systemOptionBean2 ?.mineral.toString() as String;
    }
  }
...
void save() {
  ...
  try {
    SystemOptionBean2 systemOptionBean2 = SystemOptionBean2(
      double.parse(_controlers[0].text),
      double.parse(_controlers[1].text),
      double.parse(_controlers[2].text),
    );
    DataProvider?.setSystemOptionBean2(systemOptionBean2);
  } catch (e) {
    // 处理异常的代码
    print('Error: $e');
  }
...
void getValue() {
  String message =
  (DataProvider?.getSystemOptionBean() as SystemOptionBean2).toString();
