...
class SysOptionSettingPage12 extends StatelessWidget {

  //设置该页面路由名称字符串常量
  //路由是指路由器从一个接口上收到数据包，根据数据包的目的地址进行定向并转发到另一个接口的过程
  //定义一个静态常量sysOptionSettingPageRouterName，其值为字符串"/SysOptionSettingPage"
  static const sysOptionSettingPageRouterName = "/SysOptionSettingPage12";
  ...
  const SysOptionSettingPage12({super.key});
...
class _MySysOptionSettingState extends State<MySysOptionSettingPage> {
  ...
  void initState() {
    ...
    _controlers = List.generate(14, (_) => TextEditingController());
  }
...
//当调用updateValue()函数时，会调用setState()方法更新UI界面。
  void updateValue(){
    setState(() {
      //DataProvider.getInstance() 是一个获取单例对象的方法，它返回一个DataProvider类的实例。
      // get_Ecement() 是一个方法，它返回一个包含水泥（Ecement）相关信息的对象。
      //.Ecement.toString() as String 将获取的Ecement对象的信息转换为字符串，然后赋值给_controlers[0].text，即UI控件的文本内容。

      _controlers[0].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.Ecement.             toString() as String;
      _controlers[1].text=DataProvider.getInstance()?.get_ECgCO3()?.ECaCO3.               toString() as String;
      _controlers[2].text=DataProvider.getInstance()?.get_EMgCO3()?.EMgCO3.               toString() as String;
      _controlers[3].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.ED2.                     toString() as String;
      _controlers[4].text=DataProvider.getInstance()?.get_ErawExploit()?.ErawExploit.     toString() as String;
      _controlers[5].text=DataProvider.getInstance()?.get_ErawTransport()?.ErawTransport. toString() as String;
      _controlers[6].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.Efuel.                 toString() as String;
      _controlers[7].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.Eelectric.         toString() as String;
      _controlers[8].text=DataProvider.getInstance()?.get_ErawMill()?.ErawMill.        toString() as String;
      _controlers[9].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.EfuelCrush.      toString() as String;
      _controlers[10].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.Eburn.               toString() as String;
      _controlers[11].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.Etransmit.        toString() as String;
      _controlers[12].text=DataProvider.getInstance()?.get_Qclinker_and_Fuel()?.Ecool.                toString() as String;
      _controlers[13].text=DataProvider.getInstance()?.get_Ecementmill()?.Ecementmill.    toString() as String;
    });
  }
