...
import 'package:CEP/beans/system_option_new_l_bean.dart';
...
//默认值
SystemOptionBean11? systemOptionBean11;
...
class SysOptionSettingPage11 extends StatelessWidget {

  //设置该页面路由名称字符串常量
  //路由是指路由器从一个接口上收到数据包，根据数据包的目的地址进行定向并转发到另一个接口的过程
  //定义一个静态常量sysOptionSettingPageRouterName，其值为字符串"/SysOptionSettingPage"
  static const sysOptionSettingPageRouterName = "/SysOptionSettingPage11";
  ...
  const SysOptionSettingPage11({super.key});
...
class _MySysOptionSettingState extends State<MySysOptionSettingPage> {
  final List<String> options= cementMill;
  @override
  ...
  void initState() {
    ...
    _controlers = List.generate(6, (_) => TextEditingController());
    systemOptionBean11  =
        DataProvider.getInstance()?.systemOptionBean11;
    if(systemOptionBean11 != null){
      _controlers[0].text =
      systemOptionBean11 ?.yinyaji_kw.toString() as String;
      _controlers[1].text =
      systemOptionBean11  ?.yinyaji_product.toString() as String;
      _controlers[2].text =
      systemOptionBean11 ?.shuinimo_kw.toString() as String;
      _controlers[3].text =
      systemOptionBean11 ?.shuinimo_product.toString() as String;
      _controlers[4].text =
      systemOptionBean11 ?.xuanfenji_kw.toString() as String;
      _controlers[5].text =
      systemOptionBean11 ?.xuanfenji_product.toString() as String;
    }
  }
...
void save() {
  ...
  try {
    SystemOptionBean11 systemOptionBean11 = SystemOptionBean11(
    ...
    DataProvider?.setSystemOptionBean11(systemOptionBean11);
  } catch (e) {
    // 处理异常的代码
    print('Error: $e');
  }
...
void getValue() {
  //(DataProvider?.getSystemOptionBean11() as SystemOptionBean11).toString()
  String message = (DataProvider?.getSystemOptionBean11() as SystemOptionBean11).toString();
